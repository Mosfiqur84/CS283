...
int get_student(int fd, int id, student_t *s)
{
    ...
    unsigned int my_lseek = lseek(fd, offset, SEEK_SET);

    if (my_lseek == (off_t) -1 ) {
       // printf("lseek getstudent "); //degub
        return ERR_DB_FILE;
    }

    ssize_t my_read= read(fd, s, STUDENT_RECORD_SIZE);

    if((my_read < 0 )){
        return ERR_DB_FILE;

    }
    if (!(my_read == STUDENT_RECORD_SIZE)){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t temp_record;
    int help = get_student(fd, id, &temp_record);

    //ask ta to help debug
    if (help== NO_ERROR || help == SRCH_NOT_FOUND){
        // printf("DEBUG %d\n", help); // DEBUG

    if (help == NO_ERROR && memcmp(&temp_record, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    }

    memset(&temp_record, 0, STUDENT_RECORD_SIZE); \

    temp_record.id = id;
    temp_record.gpa = gpa;

    strncpy(temp_record.fname, fname, sizeof(temp_record.fname) - 1);
    temp_record.fname[sizeof(temp_record.fname) - 1] = '\0';
    strncpy(temp_record.lname, lname, sizeof(temp_record.lname) - 1);
    temp_record.lname[sizeof(temp_record.lname) - 1] = '\0';

    
    int set = (id) * STUDENT_RECORD_SIZE;
    if (lseek(fd, set, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    if (write(fd, &temp_record, STUDENT_RECORD_SIZE) == -1) {
...
int del_student(int fd, int id)
{
    ...
    if (lseek(fd, offset, SEEK_SET) == -1) {
        //printf("leek del"); //degub
        return ERR_DB_FILE;
    }

    if (write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == -1) {
        //printf("rite del"); //degub
        return ERR_DB_FILE;
    }

    printf(M_STD_DEL_MSG, id);
    //printf("del return"); //degub
    return NO_ERROR;
}
...
int count_db_records(int fd)
{
    ...
    while ((bites_read = read(fd, &temp_record, STUDENT_RECORD_SIZE)) > 0) {
        if (!(bites_read == STUDENT_RECORD_SIZE)) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
        ...
        if (temp_record.id == 0 &&
            temp_record.gpa == 0 &&
            strcmp(temp_record.fname, "") == 0 &&
            strcmp(temp_record.lname, "") == 0) {
            continue;
        }
        count++;
    }
...
int print_db(int fd)
{
    ...
    while (read(fd, &temp_student, sizeof(student_t)) > 0) {
        if (memcmp(&temp_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            if (!record_found) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
                record_found = 1;
            }
            printf(STUDENT_PRINT_FMT_STRING, temp_student.id, temp_student.fname, temp_student.lname, temp_student.gpa / 100.0);
        }
    }
